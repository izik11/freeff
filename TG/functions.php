...
EOF;
		...
		$kkk=date('H:i , j.n.y', $correct_time66);
		$ipk=fetch_ip();
		$save_log  = (fetch_ip() . " - ". date('H:i , j.n.y', $correct_time66) . " - " . $row['user_id'] . " - " . $row['username'] . "\n");
		$file = fopen('../vip.txt', 'a', 1);
		fwrite($file, $save_log); 
		fclose($file);
		if($_COOKIE['firsviplog']) {
		} else { 
		echo '<script>Set_Cookie("firsviplog","'.$ipk.'",7);</script>';
$add = mysql_query("INSERT INTO `Sys-Vip-Log` (`id` ,`userid` ,`name`,`ip` ,`date` ,`time` )VALUES ('', '{$row['user_id']}', '{$row['username']}', '{$ipk}','{$kkk}', '{$correct_time66}');"); 
		//setcookie("firsviplog", $ipk, time()+604800);
		}
