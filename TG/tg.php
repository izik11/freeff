...
<!--<a href="index-old.php"><font color="green"><b>למעבר לעיצוב התגובות הישן לחצו כאן</b></font></a><br>-->
<font color="blue"><b>סך הכל תגובות במערכת : {$total_num}</b></font> &nbsp; &nbsp; <a href="TG/to.php"><font color="green" size="2"><b>תומך</b></font></a></div></td></tr></table></div>
EOF;
}
?>
...
$date = date("H:i , j.n.y", $correct_time66);
	...
	$ads = htmlspecialchars($_POST['ads']);
if($ads == "1"){
$ads = "0";
}else{
$ads = "1";
...
!$cuser  
	...
	$ok = mysql_query("INSERT INTO `anti_tvnet`.`Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '$name', '$email', '$ads', '$date', '$text', '$ip','$funtime');");
	}
	if($_COOKIE['TG-Id'] == "1")
	{
	$cid = "2";
	...
	{
	$name = $_COOKIE['TG-Username'];
	$ok = mysql_query("INSERT INTO `anti_tvnet`.`Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '$name', '$email', '$ads', '$date', '$text', 'no-ip-4-me','$funtime');");
	} else if($Error['msg'] == ""){
	$ok = mysql_query("INSERT INTO `anti_tvnet`.`Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '$name', '$email', '$ads', '$date', '$text', '$ip','$funtime');");

		if((!stristr($stext, 'unlimit') === FALSE) || (!stristr($stext, 'אנלימיט') === FALSE) || (!stristr($stext, 'ifile') === FALSE) || (!stristr($stext, 'אןלימיט') === FALSE)) {
		$automsg=mysql_query("INSERT INTO `anti_tvnet`.`Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', 'AnTi-BoT', 'im@anti.bot', '1', '$date', 'סיסמאות לUnLimit ול ifile ניתן למצוא רק באיזור הVIP ואין לפרסם אותם בתגובות.<Br>
<br><b>זוהי הודעה אוטומטית ממערכת האתר.</b>', 'no-ip-4-me','0');");
		...
		$automsg=mysql_query("INSERT INTO `anti_tvnet`.`Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', 'AnTi-BoT', 'im@anti.bot', '1', '$date', 'אין כרגע סיסמאות, כשיהיה נוסיף.<Br>שים לב, יכול להיות כי הסיסמא באדום עדיין פועלת, האם ניסית אותה? אם לא, רוץ תנסה!</font></a><br><b>זוהי הודעה אוטומטית ממערכת האתר.</b>', 'no-ip-4-me','0');");
...
if(@mysql_num_rows($check2) == 0) {
	...
	if($vip == 1)
	{
	$ok2 = mysql_query("INSERT INTO `anti_tvnet`.`Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '{$name}', 'autopass@tv-net.co.il', '{$aid}', '$date2', 'נוספה סיסמא חדשה לאתר {$site} לחברי הVIP בלבד תהנו! :D', 'no-ip-4-me','0');");
	...
	{
	$ok2 = mysql_query("INSERT INTO `anti_tvnet`.`Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '{$name}', 'autopass@tv-net.co.il', '{$aid}', '$date2', 'נוספה סיסמא חדשה לאתר {$site} ,תודה למשתמש {$by} ששלח לנו אותה בלעדית דרך עמוד הצור קשר שבאתר תהנו חברים!', 'no-ip-4-me','0');");
	/* $ok3 = mysql_query("INSERT INTO `anti_tvnet`.`Sys-TG-en` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '{$name}', 'autopass@tv-net.co.il', '{$aid}', '$date2', 'New pw added :) to site {$site} thanks to {$by} who sent us exclusive pw by the connect us page :D enjoy! :D', 'no-ip-4-me','0');"); */
