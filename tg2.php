...
echo <<<END
<tr>
<td class="showname" style="padding-top:2px;padding-bottom:2px; font-size:11px;" valign="top" width="100%"><div style="color:black;"><b>
{$row['id']}. 
שם: </b><a href='JavaScript:ShowHide("{$row['id']}")'>
...
!$cuser  
	...
	$ok = mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '$name', '$email', '2', '$date', '$text', '$ip','$funtime');");
	}
	if($_COOKIE['TG-Id'] == "1")
	{
	$cid = "2";
	...
	{
	$name = $_COOKIE['TG-Username'];
	$ok = mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '$name', '$email', '1', '$date', '$text', 'no-ip-4-me','$funtime');");
	} else if($Error['msg'] == ""){
	$ok = mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '$name', '$email', '0', '$date', '$text', '$ip','$funtime');");

		if((!stristr($stext, 'unlimit') === FALSE) || (!stristr($stext, 'אנלימיט') === FALSE) || (!stristr($stext, 'ifile') === FALSE) || (!stristr($stext, 'אןלימיט') === FALSE)) {
		$automsg=mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', 'AnTi-BoT', 'im@anti.bot', '1', '$date', 'סיסמאות לUnLimit ול ifile ניתן למצוא רק באיזור הVIP ואין לפרסם אותם בתגובות.<Br>
<br><b>זוהי הודעה אוטומטית ממערכת האתר.</b>', 'no-ip-4-me','0');");
		...
		$automsg=mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', 'AnTi-BoT', 'im@anti.bot', '1', '$date', 'אין כרגע סיסמאות, כשיהיה נוסיף.<Br>שים לב, יכול להיות כי הסיסמא באדום עדיין פועלת, האם ניסית אותה? אם לא, רוץ תנסה!</font></a><br><b>זוהי הודעה אוטומטית ממערכת האתר.</b>', 'no-ip-4-me','0');");
		...
		$automsg=mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', 'Radio-BOT', 'info@yuri198.co.il', '2', '$date', 'אהלן אני הבוט רדיו החדש:)<br>אם אתם מבקשים שירים תראו אותי :)<br>כששדרן, יראה הודעה זאת.<Br>עוד כמה דקות תשמע את השיר ברדיו :)<br><br><b>זוהי הודעה אוטומטית מרדיו האתר.</b>', 'no-ip-4-me','0');"); */
	}		
...
if(@mysql_num_rows($check2) == 0) {
	...
	if($vip == 1)
	{
	$ok2 = mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '{$name}', 'autopass@tv-net.co.il', '{$aid}', '$date2', 'נוספה סיסמא חדשה לאתר {$site} לחברי הVIP בלבד תהנו! :D', 'no-ip-4-me','0');");
		...
		if($site == "fileflyer") {
			...
			$add = mysql_query("INSERT INTO `Sys-User-Vip` (`user_id` ,`username` ,`password` ,`date` ,`email` ,`donate` ,`site` )VALUES ('', '{$by}', '{$pw2}', '{$time2}', '{$email}', 'vip', '{$site}');"); 
				...
				die("<script>alert('הסיסמא נוספה בהצלחה והמשתמש קיבל VIP')</script><script>window.location = ''</script>");
				}
			} else {
			$add = mysql_query("UPDATE `Sys-User-Vip` SET `date` = '{$newdate}' WHERE `Sys-User-Vip`.`user_id` ={$idvip} ;");
				...
				die("<script>alert('הסיסמא נוספה בהצלחה ולמשתמש נוסף שבוע VIP נוסף!')</script><script>window.location = ''</script>");
				}
			}
		} else { 
	...
	{
	$ok2 = mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '{$name}', 'autopass@tv-net.co.il', '{$aid}', '$date2', 'נוספה סיסמא חדשה לאתר {$site} ,תודה למשתמש {$by} ששלח לנו אותה בלעדית דרך עמוד הצור קשר שבאתר תהנו חברים!', 'no-ip-4-me','0');");
	/* $ok3 = mysql_query("INSERT INTO `Sys-TG-en` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '{$name}', 'autopass@tv-net.co.il', '{$aid}', '$date2', 'New pw added :) to site {$site} thanks to {$by} who sent us exclusive pw by the connect us page :D enjoy! :D', 'no-ip-4-me','0');"); */
