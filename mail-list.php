...
if(@mysql_num_rows($check2) == 0) {
	...
	if($vip == 1)
	{
	$ok2 = mysql_query("INSERT INTO `anti_blabla`.`Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '{$cuser}', 'autopass@freeff.co.il', '1', '$date2', 'נוספה סיסמא חדשה לאתר {$site} לחברי הVIP בלבד תהנו! :D', 'no-ip-4-me','0');");
		if($site == "fileflyer") {
			if(@mysql_num_rows($check22) == 0) {
			$add = mysql_query("INSERT INTO `Sys-User-Vip` (`user_id` ,`username` ,`password` ,`date` ,`email` ,`donate` ,`site` )VALUES ('', '{$name}', '{$pw2}', '{$time66}', '{$mail1}', 'vip', '{$site}');"); 
				if($add != "")
				{		
	...
	{
	$ok2 = mysql_query("INSERT INTO `anti_blabla`.`Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('', '{$cuser}', 'autopass@FreeFF.co.il', '1', '$date2', 'נוספה סיסמא חדשה לאתר {$site} ,תודה למשתמש {$name} ששלח לנו אותה בלעדית דרך עמוד הצור קשר שבאתר תהנו חברים!', 'no-ip-4-me','0');");
		if($site == "fileflyer") {
			if(@mysql_num_rows($check22) == 0) {
			$add = mysql_query("INSERT INTO `Sys-User-Vip` (`user_id` ,`username` ,`password` ,`date` ,`email` ,`donate` ,`site` )VALUES ('', '{$name}', '{$pw2}', '{$time66}', '{$mail1}', '{$pa}', '{$site}');"); 
				if($add != "")
				{		
				mail($mail1, "VIP at FreeFF.Co.iL - Free FileFlyer", "UserName: {$name}\n\rPassWord: {$pw2}\nEnJoY! Free FileFlyer Team.", "From: Free FileFlyer <anti.release@gmail.com>");
				die("<script>alert('הסיסמא נוספה בהצלחה והמשתמש קיבל VIP')</script><script>window.location='mail-list.php?added={$id66}&kk={$rd}';</script>");
