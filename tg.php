...
<title>Free FileFlyer - מערכת התגובות של האתר עמוד <? echo $page; ?></title>
...
<a href='TG/admin.php' target='_blank'><font size="2" color="blue">פאנל ניהול</font></a> :: 
...
<a onclick='ShowHide("Admin3")' href="#Add"><font size="2" color="red">הוסף סיסמא</font></a> :: <a href="javascript:void(0)" onclick="vip();"><font size="2" color="orange">ViP Zone</font></a><br><!--<b>שעון אמיתי : </b><span id="time"></span><br><b>שעון שרת : </b><span id="time2"></span><br><font size="2"><b>השעון בתגובות מאחר ב7 דקות | <a href="index.php"><font color="black">רענן עמוד זה!</font></a></b></font><br>-->
<!--<a href="index-old.php"><font color="green"><b>למעבר לעיצוב התגובות הישן לחצו כאן</b></font></a><br>-->
<font color="blue"><b>סך הכל תגובות במערכת : {$total_num}</b></font> &nbsp; &nbsp; <a href="TG/to.php?who=1"><font color="green" size="2"><b>תומך</b></font></a> | <a href="TG/to.php?who=2"><font color="green" size="2"><b>פרסומת</b></font></a></div></td></tr></table></div>
EOF;
}
?>
...
die("<script>alert('ההודעה שלך ארוכה מידי')</script><script>history.back()</script>");
}
if (strlen($name) > 30 && !$cuser) {
die("<script>alert('הכינוי שלך ארוך מידי')</script><script>history.back()</script>");
}
/* 
if(ord($text) == 10) { 
ie("<script>alert('רק בודקים')</script><script>history.back()</script>");
}
for($f=0; $f<strlen($text) ; $f++) {
if( ord($text) > 401) {
die("<script>alert('ניסיון נחמד')</script><script>history.back()</script>");
} */
...
/*$text=explode(' ', stripslashes(strtolower(str_replace('abc', 'aav', $text)))); */
	...
	{
	die("<script>alert('אתה לא יכול להכניס כינוי/הודעה רק עם רווחים!')</script><script>history.back()</script>");
	}
...
/*
	...
	die("<script>alert('שגיאה כללית: אנא חכה 5 דקות לשליחת התגובה הבאה')</script><script>history.back()</script>");
	}
*/
...
while($row = mysql_fetch_array($check7)) { 
	if($ip == $row['ip']) {
		die("<script>alert('אינך יכול להגיב פעמיים ברצף.')</script><script>history.back()</script>");
	}
}
...
//	if($stext17 == $stext177) {
//		die("<script>alert('אינך יכול לרשום את אותה תגובה פעמיים.')</script><script>history.back()</script>");
//	}
*/
...
if(($file2 != "il") && ($file2 != "IL") && ($file2 != "israel") && ($file2 != "Israel") && ($file2 != "ISRAEL") && ($file2 != "Il")  && ($file2 != "" ) && ($file2 != NULL))  {  
	die("<script>alert('זמנית לא ניתן להגיב עבורך')</script><script>history.back()</script>");
}

	$funtime = time();
	if(!$name || !$text)
	{
	die("<script>alert('עליך להכניס כינוי ותוכן להודעה לפני פירסומה')</script><script>history.back()</script>");
	}
	if($_COOKIE['TG-Id'] == "3")
	{
	$cid = "2";
...
/*		if((!stristr($stext, 'unlimit') === FALSE) || (!stristr($stext, 'אנלימיט') === FALSE) || (!stristr($stext, 'אןלימיט') === FALSE)) {
		$automsg=mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('0', 'AnTi-BoT', 'im@anti.bot', '1', '$date', 'סיסמאות לUnLimit ניתן למצוא רק באיזור ה VIP ואין לפרסם אותם בתגובות.<Br>
<br><b>זוהי הודעה אוטומטית ממערכת האתר.</b>', 'no-ip-4-me','0');");
		}*/
		if((!stristr($stext, 'אפשרסיסמ') === FALSE) || (!stristr($stext, 'צריךסיסמ') === FALSE) || 
		...
		$automsg=mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('0', 'AnTi-BoT', 'im@anti.bot', '1', '$date', 'אין כרגע סיסמאות, כשיהיה נוסיף.<Br>שים לב, יכול להיות כי הסיסמא באדום עדיין פועלת, האם ניסית אותה? אם לא, רוץ תנסה!</font></a><br><b>זוהי הודעה אוטומטית ממערכת האתר.</b>', 'no-ip-4-me','0');");
		...
		$automsg=mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('0', 'Radio-BOT', 'info@yuri198.co.il', '2', '$date', 'אהלן אני הבוט רדיו החדש:)<br>אם אתם מבקשים שירים תראו אותי :)<br>כששדרן, יראה הודעה זאת.<Br>עוד כמה דקות תשמע את השיר ברדיו :)<br><br><b>זוהי הודעה אוטומטית מרדיו האתר.</b>', 'no-ip-4-me','0');"); */
	}		
	...
	{
if($_POST['abs'] == "1") {
	die("<script>alert('התגובה נשלחה')</script><script>window.location = 'index.php'</script>");
} else {
	die("<script>alert('התגובה נשלחה')</script><script>history.back()</script>");
}
	}
}
...
if(@mysql_num_rows($check2) == 0) {
	...
	if($vip == 1)
	{
	$ok2 = mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('0', '{$name}', 'autopass@freeff.co.il', '{$aid}', '$date2', 'נוספה סיסמא חדשה לאתר {$site} לחברי הVIP בלבד תהנו! :D', 'no-ip-4-me','0');");
		...
		if($site == "fileflyer") {
			...
			$add = mysql_query("INSERT INTO `Sys-User-Vip` (`user_id` ,`username` ,`password` ,`date` ,`email` ,`donate` ,`site` )VALUES ('0', '{$by}', '{$pw2}', '{$time2}', '{$email}', 'vip', '{$site}');"); 
				...
				die("<script>alert('הסיסמא נוספה בהצלחה והמשתמש קיבל VIP')</script><script>history.back()</script>");
				}
			} else {
			$add = mysql_query("UPDATE `Sys-User-Vip` SET `date` = '{$newdate}' WHERE `Sys-User-Vip`.`user_id` ={$idvip} ;");
				...
				die("<script>alert('הסיסמא נוספה בהצלחה ולמשתמש נוסף שבוע VIP נוסף!')</script><script>history.back()</script>");
				}
			}
		} else { 
...
if($password22[3] != "" && $site=="rapidshare") { $site4=$password22[3]; } else { $site4=$site; }
	$ok2 = mysql_query("INSERT INTO `Sys-TG` (`id` ,`name` ,`email` ,`admin` ,`date` ,`post`,`ip`,`time` )VALUES ('0', '{$name}', 'autopass@FreeFF.co.il', '{$aid}', '$date2', 'נוספה סיסמא חדשה לאתר {$site4} ,תודה למשתמש {$by} ששלח לנו אותה בלעדית דרך עמוד הצור קשר שבאתר תהנו חברים!', 'no-ip-4-me','0');");
	...
	if($ok)
	{
	die("<script>alert('הסיסמא נוספה בהצלחה למערכת')</script><script>history.back()</script>");
	}
} else {
...
<form action="?" method="post" name="tgo">
		...
		<tr><td width="94">ע"י:</td>
			<td width="341">
		<input type="text" style="width:70%" maxlength="50" name="by" dir=ltr> מקס לקיחות: 
